...
public class Picture extends SimplePicture 
{
  ...
  public void grayscale(){
    ...
    for(Pixel[] rowArray : pixels){
        for(Pixel pixelObj : rowArray){
        i = (pixelObj.getRed()+pixelObj.getGreen()+pixelObj.getBlue())/3;
        pixelObj.setRed(i);
        pixelObj.setGreen(i);
        pixelObj.setBlue(i);
        }
  ...
  public void fixUnderWater(){
    ...
    for(Pixel[] rowArray : pixels){
        for(Pixel pixelObj : rowArray){
            if(pixelObj.getRed()<27&&pixelObj.getBlue()>154){
                if(pixelObj.getBlue()+60>255){
  ...
  public void mirrorArms(){
    ...
    for (int row = 156; row < 191; row++)
    {
      ...
      for (int col = 101; col < 171; col++)
      {
        ...
        leftPixel = pixels[row][col];      
        rightPixel = pixels[row+60][mirrorPoint - col + 240];
        rightPixel.setColor(leftPixel.getColor());
        count++;
      }
    }
        ...
        for (int row = 171; row < 196; row++)
    {
      ...
      for (int col = 237; col < 295; col++)
      {
        ...
        leftPixel = pixels[row][col];      
        rightPixel = pixels[row+65][mirrorPoint - col+107];
        rightPixel.setColor(leftPixel.getColor());
        count++;
      }
    }
  ...
  public void mirrorGull(){
    ...
    // loop through the rows
    for (int row = 227; row < 322; row++)
    {
      // loop from 13 to just before the mirror point
      for (int col = 234; col < 344; col++)
      {
